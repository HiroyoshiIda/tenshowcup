	...
	if (($member_count % 2) == 0) {
		...
		$name_str =	'<td width="150">'.
				...
				'<td width="600">'.
					$fullname_str.
				'</td>'.
				'<td rowspan="4" width="240" height="240" align="center">'.
					'<img title="画像はイメージです" src="./'.$data['tcup_member_img'].'.jpg">'.
				'</td>';
	} else {
		$tbody_border = ' border="1"';
		$name_str = 	'<td rowspan="4" width="240" height="240" align="center">'.
					'<img title="画像はイメージです" src="./'.$data['tcup_member_img'].'.jpg">'.
				'</td>'.
				'<td width="150">'.
					'名前'.
...
if (!array_key_exists(2, $attendee_list)) {
	// 今回はチャイあんが補欠.
	$tbody_border = ' border="1"';
	$name_str = 	'<td rowspan="3" width="240" height="240" align="center">'.
				'<img title="画像はイメージです" src="./cya.jpg">'.
			'</td>'.
			'<td width="150">'.
				'名前'.
			'</td>'.
		...
		'<table border="1">',
			'<tbody'.$tbody_border.'>',
				...
				'<tr>',
					...
					'</td>',
					'<td>',
						"某氏の他大会出向により今回の主催者（＆控え）です。<br />",
						"至らぬ点も多々あると思いますが、楽しんでいただければ幸いです。<br />",
					'</td>',
				'</tr>',
			'</tbody>',
		'</table>';
