...
// -------------
	...
	// 戦績取得.
	if ($member_id != $opp_id) {
		$vs_result = Get_TopenVsResult($dbo, $member_id, $opp_id);			// 対戦相手情報を受け取る.
	}
					...
					foreach ($result_data as $result_cnt => $data) {
						...
						if ($rank_cnt == 1) {
							...
							$show_str .=	'<td bgcolor="'.$show_color[$color_no].'">'.
											...
											'</td>'.
											'<td align="center" bgcolor="'.$show_color[$color_no].'">'.
												$_G_Existence_text[$data['is_red']].									// 赤.
											'</td>'.
											'<td align="center" bgcolor="'.$show_color[$color_no].'">'.
												$_G_Existence_text[$data['is_yakitori']].								// ヤキトリ.
											'</td>';
						}
...
echo
	...
	'<head>',
	  ...
	  '<title>天翔杯－オープン戦</title>',
	'</head>',
	'<body bgcolor="#FFFAFA">',
	'<div style="margin-left: 60px; margin-right: 60px">',
		'<br>',
		'<center><FONT size=+3><b>対個人戦績</b></FONT><br></center>',
		'<hr width="100%">',
		'<FONT size=+1>',
		'<center>',
